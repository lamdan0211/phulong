// Slider functionality - chỉ chạy khi slider HTML tồn tại
let currentSlide = 0;
let slides = null;
let totalSlides = 0;
let tabs = null;
...
// Biến riêng cho slider clubhouse amenities (trong popup)
...
let clubhouseAmenitiesActivePanels = new Set(); // Set để lưu các panel đang active (ở bên phải)
let clubhouseAmenitiesPanelsOnRight = new Set(); // Set để track panel nào đang ở bên phải

function initSlider() {
    // Init slider công viên (main slider, không phải popup)
...
// Hàm riêng cho slider clubhouse amenities (trong popup)
function initClubhouseAmenitiesSlider() {
    ...
    clubhouseAmenitiesCurrentSlide = -1; // Ban đầu không có panel nào active
    clubhouseAmenitiesPreviousSlide = -1;
    clubhouseAmenitiesActivePanels.clear(); // Reset active panels
    clubhouseAmenitiesPanelsOnRight.clear(); // Reset panels on right
    
    // Ban đầu tất cả slide đều không active
    modalSlides.forEach((slide) => {
        slide.classList.remove('active');
    });
    
    // Ban đầu tất cả panel-item đều ở bên trái
    ...
    // Update slider để sắp xếp panels đúng
    ...
    // Add keyboard navigation (only when popup is active)
    ...
    function handleSwipe() {
        if (touchEndX < touchStartX - 50) {
            moveClubhouseAmenitiesSlide(1); // Swipe left
        }
        if (touchEndX > touchStartX + 50) {
            moveClubhouseAmenitiesSlide(-1); // Swipe right
        }
    }
...
function updateClubhouseAmenitiesSlider() {
    ...
    // Calculate slide direction (positive = next/right, negative = prev/left)
    const slideDirection = clubhouseAmenitiesCurrentSlide - clubhouseAmenitiesPreviousSlide;
    
    // Update slide transform with transition
    ...
    // Update panels - active panels move to right, sync with slide transition
    const panels = mainSlider.querySelectorAll('.panel-item');
    ...
    // Sắp xếp lại các panel: panel active gần nhất có thể ở trái hoặc phải, các panel khác sắp xếp đối xứng
    let leftPosition = 0;
    let rightPosition = 0;
    const panelWidth = 50;
    
    // Tách panel active gần nhất (current slide) và các panel khác
    ...
    // Sắp xếp các panel không phải current: ở trái hoặc phải
    ...
    // Sắp xếp panels bên trái (trước current panel nếu có)
    panelsOnLeft.forEach(({ panel, slide }) => {
        const text = panel.querySelector('.vertical-text');
        panel.classList.remove('active');
        if (text) text.classList.remove('active'); // Không có chữ vàng
    ...
    // Panel active gần nhất (current slide): có chữ vàng, có thể ở trái hoặc phải
    if (currentPanel !== null) {
        ...
        if (currentPanelElement) {
            ...
            if (text) text.classList.add('active'); // Có chữ vàng
            
            // Xác định panel này đang ở bên nào
    ...
    // Sắp xếp các panel khác bên phải (không phải current)
...
function moveClubhouseAmenitiesSlide(direction) {
    ...
    // Sử dụng goToClubhouseAmenitiesSlideFromTab để có logic trượt đối xứng
...
function goToClubhouseAmenitiesSlideFromTab(index) {
    ...
    // Click vào panel nào thì panel đó active, trượt sang đối xứng, có chữ vàng
    const isOnRight = clubhouseAmenitiesPanelsOnRight.has(index);
    
    if (isOnRight) {
        // Panel đang ở bên phải -> trượt sang trái, vẫn active, có chữ vàng
        clubhouseAmenitiesPanelsOnRight.delete(index);
        clubhouseAmenitiesActivePanels.delete(index); // Remove khỏi Set active (bên phải)
    } else {
        // Panel đang ở bên trái -> trượt sang phải, active, có chữ vàng
        clubhouseAmenitiesPanelsOnRight.add(index);
        clubhouseAmenitiesActivePanels.add(index); // Add vào Set active (bên phải)
    }
    
    // Panel được click luôn trở thành current slide (có chữ vàng)
    clubhouseAmenitiesCurrentSlide = index;
    
    // Update slide - chỉ hiển thị slide của panel active gần nhất
    const mainSlider = popup.querySelector('.clubhouse-slider');
    if (mainSlider) {
        const modalSlides = mainSlider.querySelectorAll('.slide');
...
function updateSlider() {
    ...
    // Legacy support for left-panel, right-panel
...
// ============================================
// Scroll Animations (Essensia Style)
// ============================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle window resize for info slider (mobile/desktop switch)
    let resizeTimeout;
    window.addEventListener('resize', () => {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(() => {
            if (infoSliderTrack && totalInfoSlides > 0) {
                // Reinitialize slider to update dots count
...
function toggleMobileNav() {
    ...
    if (navContainer && mobileNavButton) {
...
let congVienActivePanels = new Set(); // Set để lưu các panel đang active (ở bên phải)
let congVienPanelsOnRight = new Set(); // Set để track panel nào đang ở bên phải
...
function initCongVienSlider() {
    ...
    congVienCurrentSlide = -1; // Ban đầu không có panel nào active
    congVienPreviousSlide = -1;
    congVienActivePanels.clear(); // Reset active panels
    congVienPanelsOnRight.clear(); // Reset panels on right
    ...
    // Set slide đầu tiên active và panel đầu tiên active
    if (modalSlides.length > 0) {
        modalSlides[0].classList.add('active');
        congVienSlides.style.transform = 'translateX(0%)';
        
        // Set panel đầu tiên active
        if (panels.length > 0) {
...
function updateCongVienSlider() {
    ...
    // Update slide transform with transition
    // Slide trượt sang trái khi next (translateX âm), sang phải khi prev (translateX dương)
...
function goToCongVienSlideFromTab(index) {
    ...
    if (isOnRight) {
        ...
        congVienActivePanels.delete(index); // Remove khỏi Set active (bên phải)
    } else {
        // Panel đang ở bên trái -> trượt sang phải, active, có chữ vàng
        congVienPanelsOnRight.add(index);
        congVienActivePanels.add(index); // Add vào Set active (bên phải)
    }
...
function moveCongVienSlide(direction) {
    ...
    // Prevent looping - disable at ends
    if (newIndex < 0) {
        return;
    ...
    // Sử dụng goToCongVienSlideFromTab để chuyển slide
    goToCongVienSlideFromTab(newIndex);
}

function openCongVienPopup() {
    console.log('openCongVienPopup called');
    const popup = document.getElementById('congVienPopup');
...
let clubhouseActivePanels = new Set(); // Set để lưu các panel đang active (ở bên phải)
let clubhousePanelsOnRight = new Set(); // Set để track panel nào đang ở bên phải
...
function initClubhouseSlider() {
    ...
    clubhouseCurrentSlide = -1; // Ban đầu không có panel nào active
    clubhousePreviousSlide = -1;
    clubhouseActivePanels.clear(); // Reset active panels
    clubhousePanelsOnRight.clear(); // Reset panels on right
    ...
    // Set slide đầu tiên active nhưng không set panel active
    if (modalSlides.length > 0) {
        modalSlides[0].classList.add('active');
...
function openClubhousePopup() {
    console.log('openClubhousePopup called');
    const popup = document.getElementById('clubhousePopup');
    console.log('Popup element:', popup);
    if (popup) {
        popup.classList.add('active');
        document.body.style.overflow = 'hidden'; 
        console.log('Clubhouse popup opened');
        ...
        // Initialize với floor 1
        switchFloor(1);
        
        // Initialize slider when popup opens
        setTimeout(() => {
        ...
        console.error('Clubhouse popup element not found!');
    }
}
...
function closeClubhousePopup() {
    ...
    if (popup) {
        ...
        // Clear autoplay interval when popup closes
        if (clubhouseAmenitiesAutoPlayInterval) {
            clearInterval(clubhouseAmenitiesAutoPlayInterval);
...
function switchFloor(floorNumber) {
    ...
    // Normalize floorNumber (convert to string for consistent comparison)
    const normalizedFloor = String(floorNumber);
    
    // Update floor-tab-button text và active state
    ...
    if (activeDescription) {
        activeDescription.classList.add('active'); // Hiển thị cùng với default description
    }
...
// Handle floor tab click - toggle dropdown on mobile, switch floor on desktop
function handleFloorTabClick(floorNumber, event) {
    ...
    // Get the actual button element (could be clicked element or its parent)
    ...
    // On mobile, if clicking active button and it's the current floor, toggle dropdown
    if (window.innerWidth <= 768 && isButtonClick && isCurrentFloor) {
        if (floorTabs) {
            floorTabs.classList.toggle('dropdown-open');
        }
    } else {
        // Switch floor (either from button click on different floor, or from dropdown item)
...
// Toggle dropdown for mobile floor tabs
...
function moveRoleFrameSlide(direction) {
    console.log('moveRoleFrameSlide called with direction:', direction);
    const roleModal = document.getElementById('rolePopup');
    if (!roleModal) {
        console.warn('moveRoleFrameSlide: roleModal not found');
        return;
    }
    
    const dragState = sliderDragStates['roleFrameSlider'];
    if (dragState && dragState.isDragging) {
        console.log('moveRoleFrameSlide: Currently dragging, ignoring click');
        return;
    }
    
    if (!roleFrameSlides || totalRoleFrameSlides === 0) {
        console.log('moveRoleFrameSlide: Initializing slider');
        initRoleFrameSlider();
        return;
    }
    ...
    console.log('moveRoleFrameSlide: Moving to slide', currentRoleFrameSlide);
    updateRoleFrameSlider();
}
...
// Role Second Slider Drag Variables (kept for backward compatibility, but will use new system)

function initRoleSecondSlider() {
    const roleModal = document.getElementById('rolePopup');
    if (!roleModal) return false;
    
    // Chỉ tìm trong container của slider thứ 2
    ...
    // Setup drag functionality using universal system
...
function openRoleModal() {
    ...
    if (popup) {
        ...
        console.error('Role popup element not found!');
    }
}
...
// Role Second Modal Functions - Completely separate from RoleModal
// First Slider for Role Second Modal
...
function moveRoleSecondModalFirstSlide(direction) {
    ...
    // Don't loop - prevent going beyond boundaries
...
function openRoleSecondModal() {
    console.log('openRoleSecondModal called');
    const popup = document.getElementById('roleSecondPopup');
    console.log('Popup element:', popup);
    if (popup) {
        popup.classList.add('active');
        document.body.style.overflow = 'hidden'; 
        console.log('Role Second popup opened');
        
        // Enable all buttons when modal opens
        ...
        console.error('Role Second popup element not found!');
    }
}
...
// Switch floor tabs in Role modal
...
// Switch optimal floor by clicking on the left image (for shophouse section in rolePopup)
function switchOptimalFloor(floorNumber) {
    ...
    // Update left images - hide all, show selected
    ...
    // Update right images - hide all, show selected
    ...
    // Update text images - hide all, show selected
...
// Switch optimal floor for Toam section (in roleSecondPopup)
...
function initInfoSlider() {
    ...
    // Calculate total pages: mobile = 1 item per page, desktop = 3 items per page
    const itemsPerPage = isMobile ? 1 : 3;
    const totalPages = Math.ceil(totalInfoSlides / itemsPerPage);
    
    // Update pagination dots dynamically
    if (paginationContainer) {
        paginationContainer.innerHTML = '';
        if (totalPages > 1) {
            for (let i = 0; i < totalPages; i++) {
                ...
                // Sử dụng cả click và touch để đảm bảo hoạt động tốt trên mobile
                const dotIndex = i; // Capture index để tránh closure issue
                ...
                dot.addEventListener('touchend', (e) => {
                    ...
                    if (touchDuration < 300) {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('Dot touch:', dotIndex);
                        goToInfoSlide(dotIndex);
                    }
                }, { passive: false });
    ...
    // On mobile, ensure transform is removed and native scroll works
    if (isMobile) {
        if (infoSliderTrack) {
            infoSliderTrack.style.transform = 'none';
            infoSliderTrack.style.transition = 'none';
            // Đảm bảo track có thể scroll được
        ...
        if (container) {
            ...
            container.addEventListener('touchstart', (e) => {
                // Cho phép touch scroll
            }, { passive: true });
            container.addEventListener('touchmove', (e) => {
                // Cho phép touch scroll
            }, { passive: true });
            
            // Listen to scroll events to update active dot on mobile
            let scrollTimeout;
...
function updateInfoSlider() {
    ...
    // Check if we're on mobile (use native scrolling instead of transform)
    const isMobile = window.innerWidth <= 560;
    
    if (isMobile) {
        // On mobile, don't use transform - let native scroll handle it
    ...
    // Calculate translateX: move by 3 items (100% of viewport)
    const translateX = -currentInfoSlide * 100;
    infoSliderTrack.style.transform = `translateX(${translateX}%)`;
    
    // Use the new updatePaginationDots function
...
function goToInfoSlide(index) {
    ...
    // Stop autoplay when user clicks on dot
    if (infoAutoPlayInterval) {
        clearInterval(infoAutoPlayInterval);
        infoAutoPlayInterval = null;
    }
    ...
    // On mobile, scroll to the card instead of using transform
    if (isMobile) {
        const container = document.querySelector('.info-slider-container');
        ...
        if (container && cards.length > index) {
            ...
            if (targetCard) {
                // Calculate scroll position - scroll card vào giữa màn hình
                const containerRect = container.getBoundingClientRect();
                const cardRect = targetCard.getBoundingClientRect();
                const containerScrollLeft = container.scrollLeft;
                
                // Calculate scroll position - scroll card vào giữa container
                const cardLeft = targetCard.offsetLeft;
                const cardWidth = targetCard.offsetWidth;
                const containerWidth = container.offsetWidth;
                // Scroll để card ở giữa container
                ...
                // Fallback nếu scrollTo không hoạt động
                setTimeout(() => {
    ...
    // Restart autoplay after 8 seconds of inactivity (desktop only)
    if (!isMobile) {
        setTimeout(() => {
            if (!infoAutoPlayInterval) {
                startInfoAutoPlay();
...
function startInfoAutoPlay() {
    // Don't autoplay on mobile - use native scroll instead
    const isMobile = window.innerWidth <= 560;
    if (isMobile) {
        return;
    }
    ...
    // Only start autoplay if there are multiple pages (desktop: 3 items per page)
    const itemsPerPage = 3;
    const totalPages = Math.ceil(totalInfoSlides / itemsPerPage);
    
    if (totalPages > 1) {
        infoAutoPlayInterval = setInterval(() => {
            // Don't use moveInfoSlide to avoid stopping autoplay
            if (!infoSliderTrack || totalInfoSlides === 0) return;
...
function initMasterPlanMapScroll() {
    ...
    if (!mapWrapper) {
        console.log('Map wrapper not found');
        return;
    }
    
    // Tìm container chứa SVG (container bên trong map-wrapper)
    ...
    // nav-scroll-map giờ nằm ngoài map-wrapper, trong master-plan-map-container
    const navScrollMap = mapContainer.querySelector('.nav-scroll-map');
    if (!navScrollMap) {
        console.log('Nav scroll map not found');
        return;
    }
    
    // Tìm các nút - có thể là div hoặc img
    let prevButton = navScrollMap.querySelector('div:first-child');
    let nextButton = navScrollMap.querySelector('div:last-child');
    
    // Nếu không tìm thấy div, thử tìm img
    ...
    if (!prevButton || !nextButton) {
        console.log('Buttons not found', { prevButton, nextButton, navScrollMap });
        return;
    }
    
    // Scroll amount - use viewport width for better UX
    const scrollAmount = window.innerWidth * 0.8;
    
    // Previous button (scroll left) - scroll container chứa SVG
    prevButton.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('Prev button clicked, scrolling left', { svgContainer, scrollAmount });
        if (svgContainer) {
    ...
    // Next button (scroll right) - scroll container chứa SVG
    nextButton.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('Next button clicked, scrolling right', { svgContainer, scrollAmount });
        if (svgContainer) {
            svgContainer.scrollBy({
                left: scrollAmount,
                behavior: 'smooth'
    ...
    console.log('Master plan map scroll initialized');
}

// Frame Depiction Slider
let currentFrameSlide = 0;
let frameSlides = null;
let totalFrameSlides = 0;

// Universal Drag System for All Sliders
const sliderDragStates = {};

// Initialize drag state for a slider
...
// Setup drag functionality for any slider
function setupSliderDrag(sliderId, trackElement) {
    ...
    // Don't clone track element as it removes onclick handlers
    // Just add event listeners directly
...
function handleSliderDragStart(e, sliderId) {
    ...
    // Disable navigation buttons for modal sliders
...
function handleSliderDragMove(e, sliderId) {
    ...
    // Get total slides to calculate boundaries
    let totalSlides = 0;
...
function handleSliderDragEnd(e, sliderId) {
    ...
    // Enable navigation buttons for modal sliders
    if (sliderId === 'roleFrameSlider' || sliderId === 'roleSecondSlider' || 
        sliderId === 'roleSecondModalFirstSlider' || sliderId === 'roleSecondModalSecondSlider') {
    ...
    // Get total slides to check boundaries
    let totalSlides = 0;
    const currentSlide = state.getCurrentSlide();
    if (sliderId === 'roleFrameSlider') {
        totalSlides = totalRoleFrameSlides;
...
// Enable all navigation buttons when modal opens
function enableAllRoleModalButtons() {
    const roleModal = document.getElementById('rolePopup');
    if (!roleModal) {
        console.warn('enableAllRoleModalButtons: roleModal not found');
        return;
    }
    
    const allButtons = roleModal.querySelectorAll('.frame-slider-arrow');
    console.log('enableAllRoleModalButtons: Found', allButtons.length, 'buttons');
    ...
    allButtons.forEach(btn => {
        ...
        console.log('Button enabled:', btn, 'onclick:', btn.getAttribute('onclick'));
    });
}

// Enable all navigation buttons for roleSecondPopup when modal opens
function enableAllRoleSecondModalButtons() {
    const roleSecondModal = document.getElementById('roleSecondPopup');
    if (!roleSecondModal) {
        console.warn('enableAllRoleSecondModalButtons: roleSecondModal not found');
        return;
    }
    
    const allButtons = roleSecondModal.querySelectorAll('.frame-slider-arrow');
    console.log('enableAllRoleSecondModalButtons: Found', allButtons.length, 'buttons');
...
// Update button states based on current slide position
...
// Frame Slider Drag Variables (kept for backward compatibility, but will use new system)
...
function updateFrameSlider() {
    ...
    // Chỉ update slider ở trang chính
...
function switchFrameTab(tabName) {
    ...
    // Chỉ hoạt động với slider ở trang chính
    const mainFrameSlider = document.querySelector('.frame-depiction-section .frame-slider-container');
...
// Handle frame tab click - toggle dropdown on mobile, switch tab on desktop
function handleFrameTabClick(tabName, event) {
    ...
    // Get tabName from the actual button element
    ...
    // On mobile, if clicking active button and it's the current tab, toggle dropdown
    if (window.innerWidth <= 768 && isButtonClick && isCurrentTab) {
        frameTabNav.classList.toggle('dropdown-open');
        console.log('Toggle dropdown:', frameTabNav.classList.contains('dropdown-open'));
    } else {
        // Switch tab (either from button click on different tab, or from dropdown item)
