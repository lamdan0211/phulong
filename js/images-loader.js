...
import { IMAGE_MAPPINGS } from './content-mappings.js';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (imagesData) {
            ...
            // Apply dynamic images from content-mappings (new structure)
            applyDynamicImages(imagesData.dynamicImages || {});
...
/**
 * Apply dynamic images từ IMAGE_MAPPINGS array
 */
function applyDynamicImages(dynamicImages) {
    ...
    IMAGE_MAPPINGS.forEach(item => {
        const imageUrl = dynamicImages[item.key];

        // Chỉ apply nếu có giá trị và là Base64
        if (!imageUrl || !imageUrl.startsWith('data:')) {
            return;
        }
        ...
        try {
            ...
            if (item.mode === 'background') {
                ...
                appliedCount++;
            } else if (item.mode === 'src' || target.tagName === 'IMG') {
                target.src = imageUrl;
                appliedCount++;
            }
        } catch (error) {
            console.warn(`⚠️ Could not apply ${item.key}:`, error);
        }
    });

    if (appliedCount > 0) {
        console.log(`✅ Dynamic images applied: ${appliedCount}`);
    }
}
