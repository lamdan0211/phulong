...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const content = await loadContentFromLocal();
    if (content) {
...
async function loadContentFromLocal() {
  try {
    const response = await fetch(`/api/Data/content?t=${Date.now()}`);
    if (response.ok) {
...
function applyNewsContent(news) {
  ...
  // Sort by order
  const sortedItems = [...news.items].sort((a, b) => (parseInt(a.order) || 999) - (parseInt(b.order) || 999));
